...
object Simplifier {
  ...
  def simplifyBinExpr(expr:BinExpr):Node={
    ...
    val simplifiers = List(
      ...
      evaluateConstants,
      simplifyBinExprMinuses,
      simplifyMultiplication,
      simplifyDivisionAndCommutativity,
  ...
  val simplifyPowers: PartialFunction[BinExpr,Node] ={
    case BinExpr("*",BinExpr("**",a,b),BinExpr("**",c,d)) if a.customEquals(c)=>
      simplifyBinExpr(BinExpr("**",a,BinExpr("+",b,d)))
  ...
  val simplifyMultiplication: PartialFunction[BinExpr,Node] ={



    case BinExpr("-",BinExpr("*",a,b),c) if a.customEquals(c)=>
      simplifyBinExpr(BinExpr("*",a,BinExpr("-",b,IntNum(1))))

    case BinExpr("-",c,BinExpr("*",a,b)) if a.customEquals(c)=>
      simplifyBinExpr(BinExpr("*",a,BinExpr("-",IntNum(1),b)))
  ...
  val simplifyDivisionAndCommutativity: PartialFunction[BinExpr,Node]={
  ...
  val simplifyBinExprMinuses: PartialFunction[BinExpr,Node] ={

    case BinExpr("+",Unary("-",leftNode),Unary("-",rightNode))  =>
      Unary("-",BinExpr("+",leftNode,rightNode))

    case BinExpr("+",Unary("-",leftNode),rightNode)  =>
      simplifyBinExpr(BinExpr("-",rightNode,leftNode))

    case BinExpr("+",leftNode,Unary("-",rightNode))  =>
      simplifyBinExpr(BinExpr("-",leftNode,rightNode))

  }

  val simplifyConcatenation: PartialFunction[BinExpr,Node] ={
